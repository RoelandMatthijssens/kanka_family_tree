...
app.get('/characters', async (req, res) => {
  ...
  const relations = await kanka.get_relations()

  characters.map(character => {
    character.populate_relations(relations, characters)
  })
  // res.send(JSON.safeStringify(characters))
  res.send(tree_html(characters,2))
})
...
function tree_html(characters) {
  return `
    ...
    window.onload = function () {
        ...
        chart.load(${JSON.stringify(characters.map(i => {return i.to_chart_data()}))});
    };
        </script>
    </body>
    </html>
